...
| ID     | タイトル | 条件 |
|--------|----------|------|
| R3W001 | Prefer generic R3EventAttribute\<T\> | C# 11 以降の言語バージョンで非ジェネリック版 `R3EventAttribute(typeof(T))` を使用している場合 |

- **R3W001** の報告位置: 属性付加されているクラス宣言ではなく、`[R3EventAttribute(typeof(T))]` という記述（`AttributeSyntax`）の位置に報告する。
- C# 10 以前では非ジェネリック版が唯一の選択肢であるため、R3W001 は発行されない。
- R3E001〜R3E004 のいずれかが発行される場合、R3W001 は発行されない（エラーが優先される）。

**9.3. コードフィックス (R3W001)**

R3W001 に対して、IDE のクイックフィックス（`CodeFixProvider`）が提供される。
